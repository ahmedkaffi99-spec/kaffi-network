...
PLUSIEURS PICKS SUR LE MÊME MATCH AUTORISÉS : si un match a plusieurs tendances indépendantes qui respectent chacune la règle 1 (ex: over2.5=88% ET wins_home=85% sur le même match), propose les DEUX comme picks candidats séparés plutôt que de forcer un choix arbitraire — ce n'est pas parce qu'un type de pari est solide qu'un autre l'est moins, chaque prédiction reste indépendante. Le Sélecteur de cotes se charge ensuite de n'en garder qu'un seul par match dans un même combiné (deux paris sur le même match dans un même coupon seraient corrélés, pas diversifiés).
